...
function main()
    l_result = utils.inputbox("通知列表是?(格式：abc|bcd|dca)", "commlist", GetVariable("commlist"), "宋体", "12")
        ...
        function()
            if currentLocate == nil then
                ...
                elseif locl.where:find("兰州城大渡口") then
                    currentLocate = "兰州渡口"
                elseif locl.where:find("长安城陕晋渡口") or locl.where:find("长安城渡船") then
                    currentLocate = "陕晋渡口"
                elseif locl.where:find("黄河流域大渡口") or locl.where:find("黄河流域渡船") then
                    currentLocate = "黄河流域大渡口"
                end
            end

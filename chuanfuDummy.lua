...
function main()
    l_result = utils.inputbox("通知列表是?(格式：abc|bcd|dca)",
                              "commlist", GetVariable("commlist"), "宋体",
                              "12")
...
function boatout() boatmsg("出发时间:") end

function boatarrive() boatmsg("到达时间:") end
...
-- /SetVariable("commlist", "rainbow|hnwbh|wbh|xiaohama|parrot|qqqqqqqq|bubble|lakesi|beggar|root|tjkl|android|yuji|aion|lfw|tzssp|zruo|zjj|gjy|hfx|kickall|dzyu|mjjjjlll|tanwr|asura|azi|muxue|daji|dragon|yelang|tuiop|kickcool|juvair|minds|cdm|thaxthx|gouglz|xuxiake")
...
tmp = tmp or {}
hp = {exp = 0}
function idle() create_timer_s('idle', 30, 'idle_do') end

function idle_do() exe('yell boat;poem') end
job = {name = "chuanfu"}

...
huashan_find=function(n,l,w)
   ...
   if string.find(job.where,"洗象池") or string.find(job.where,"地下黑拳市") or string.find(job.where,'绝情谷石窟') then
...
huashan_yls_trigger=function()
    ...
    create_trigger_t('ylshead1','^(> )*你把 "action" 设定为 "华山任务完成" 成功完成。$','','huashan_yls_give')
...
huashan_yls_give=function()
	...
	create_trigger_t('huashan_yls1','^(> )*(你的令牌呢|你还没有去找恶贼，怎么就来祭坛了？)','','huashan_yls_fail')

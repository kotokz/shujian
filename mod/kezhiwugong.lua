...
function kezhiwugongkill(p_num)
	...
	if kflag[p_num]==2 and not tempv then
		--print("heqi="..heqi)
		if pkheqi==1 and (job.name=='diemenglou' or job.name=='pk' or job.name == 'guanfu' or job.name == 'hubiao') then
		...
		if pkheqi==1 and (job.name=='diemenglou' or job.name=='pk' or job.name == 'guanfu' or job.name == 'hubiao') then
		   ...
		   elseif job.name=='songmoya' then
		        exe('set wimpycmd smybuff\\mydebuff\\pkpfm\\mypfm\\hp;set wimpy 100')
		   else
...
function kezhiwugongValue(kzValue,i,kezhiwugongName)
	...
	if job.name=='songmoya' then
		if i==1 then
			if sxjob.skills1~=nil and sxjob.skills1=='散花掌' and kezhiwugongName=='般若掌' then kzValue='正' end
		end
		if i==2 then
			if sxjob.skills2~=nil and sxjob.skills2=='散花掌' and kezhiwugongName=='般若掌' then kzValue='正' end
		end
	end
...
function kezhiwugongcheck(i,n,l,w)
  ...
  if job.name == 'pk' then
	...
	if w[2]~=nil then
		if string.find(w[2],"你") and string.find(w[2],"使出桃花岛「狂风绝技」") then
           exe('jiali max')
	    end
...
function kezhiwugongSetPerform(tempatt,i,targetid)
	...
	elseif job.name == 'songmoya' then
	   ...
	   if yptteam>6 and yptteam<19 then
		  create_alias('kezhiwugongpfm','kezhiwugongpfm','alias pppp'..i..' '..GetVariable(pk_valuecmd[tempatt])..';'..GetVariable("smybuff")..';'..GetVariable("smy_pfm1")..' '.. targetid)
	      exe('kezhiwugongpfm')
		  exe('set 兰花手 蝶舞式;jiali max')
	   end
	   if yptteam>18 and yptteam<23 then
		  exe('jiali 1;set jybgz 1;set hmg 逆转')
		  create_alias('kezhiwugongpfm','kezhiwugongpfm','alias pppp'..i..' '..GetVariable(pk_valuecmd[tempatt])..';'..GetVariable("smybuff")..';'..GetVariable("smy_pfm3")..' '.. targetid)
	      exe('kezhiwugongpfm')
		  exe('unset debuff_boost')
	   end
	   if yptteam>22 and yptteam<26 then
		  exe('jiali 1;set jybgz 1;set hmg 逆转')
		  create_alias('kezhiwugongpfm','kezhiwugongpfm','alias pppp'..i..' '..GetVariable(pk_valuecmd[tempatt])..';'..GetVariable("smybuff")..';'..GetVariable("smy_pfm2")..' '..targetid..';'..GetVariable("smy_pfm3")..' '.. targetid..';jiali 1')
	      exe('kezhiwugongpfm')
		  exe('unset debuff_boost')
	   end
	  end
...
function kezhiwugongsuccess(kezhi_cmd,k_order,npci)
	...
	if kezhisuccess>=npc_num or table.getn(npcdienum)>0 then
		local tflag=1
		for i=2,npc_num do
			if npc_order[i]~=nil and npc_order[tflag]~=nil and npc_order[i]>npc_order[tflag] then

...
chat_trigger = function()
    ...
    create_triggerex_lvl('chat3',
                         ...
                         '', 'chat_unique', 99)
    create_triggerex_lvl('chat4',
                         "^(> )*(\\! )*【谣言】某人：有人带着(\\D*)在(\\D*)一带出现！$",
                         '', 'rumor_unique', 99)
    create_triggerex_lvl('chat5',
                         "^(> )*(\\! )*【谣言】某人：(\\D*)从(\\D*)处得到了一(件|条|支|把|副|根|柄|对|本|个)(\\D*)！$",
                         '', 'chat_Unique', 99)
...
function userSave()
    ...
    if type(l_user) == "string" then SetVariable("muduser", l_user) end
end
function userGet()
    ...
    for i = 1, table.getn(l_set) do
        ...
        if l_id[1] then MudUser[l_id[1]] = l_id[2] end
    end
end
function userAdd(set) for p in pairs(set) do MudUser[p] = set[p] end end
    ...
    if first_time and not w then
        ...
        if not w then
            ColourNote("white", "red",
                       "Can't open chat world file: " .. filename)
...
function chatUser(name, line, wildcards, styles)
    ...
    if score.id and score.id == 'ptbx' then return false end
...
function chats_log(logs, color, bcolor)
    ...
    if flag.log and flag.log == "yes" then
        ...
        if job.name == 'xueshan' or job.name == 'gaibang' then
            c_text = 'darkred'
        end
        if job.name == 'gblu' or job.name == 'songxin2' then
            c_text = 'darkviolet'
        end
...
function chat_unique(name, line, wildcards, styles)
    ...
    if score.id and score.id ~= 'xxf' then return false end
    ...
    if MudUser[wildcards[3]] then
        ...
        if not MudUnique[wildcards[5]] then flag.unique = 2 end
...
function chat_Unique(name, line, wildcards, styles)
    ...
    if MudUser[wildcards[3]] then
        if MudUser[wildcards[4]] then
            if MudUnique[wildcards[6]] and MudUnique[wildcards[6]] ~=
                wildcards[3] then chatUniqueEarse(wildcards[6]) end
            if not MudUnique[wildcards[6]] then flag.unique = 2 end
...
function chatUniqueEarse(p_unique)
    ...
    flag.unique = 1
    Mchat.time = os.time()
    exe('chat* 掏出一个小本本划掉了上面的' .. p_unique ..
            MudUnique[p_unique] .. MudUser[MudUnique[p_unique]] ..
            '几个字。')
end
function chatUniqueChat(p_unique)
    ...
    if (os.time() - Mchat.time < 30 or math.random(1, 8) > 4) and flag.unique and
        flag.unique == 0 then return false end
    Mchat.time = os.time()
    if flag.unique and flag.unique == 2 then
        if math.random(1, 3) == 1 then
            exe(
                'chat* 掏出一个小本本飞快的写写画画，隐约可以看到歪歪扭扭的' ..
                    p_unique .. MudUnique[p_unique] ..
                    MudUser[MudUnique[p_unique]] .. '几个字。')
        elseif math.random(1, 3) == 2 then
            exe('chat* 慌忙抽出一张手纸，写上' .. p_unique ..
                    MudUnique[p_unique] .. MudUser[MudUnique[p_unique]] ..
                    '几个字，又把它贴在一个笔记本后面。')
        else
            exe(
                'chat* 拿出一个红皮本，翻了几页，端端正正的写上' ..
                    p_unique .. MudUnique[p_unique] ..
                    MudUser[MudUnique[p_unique]] .. '几个大字。')
        end
    elseif flag.unique and flag.unique == 1 then
        exe('chat* 又在后面添上' .. p_unique .. MudUnique[p_unique] ..
                MudUser[MudUnique[p_unique]] .. '几个字。')
    else
        exe(
            'chat* 掏出一个小本本在' .. p_unique .. MudUnique[p_unique] ..
                MudUser[MudUnique[p_unique]] .. '几个字上又描了一遍。')
    end
    flag.unique = 0
end
...
function getRandomIndex(p_set)
    if type(p_set) ~= "table" then return false end

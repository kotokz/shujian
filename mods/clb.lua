...
function clbTrigger()
    ...
    create_trigger_t('clbAsk1', "^(> )*你向贝海石打听有关", '',
                     'clbAsk')
    create_trigger_t('clbAsk2', "^(> )*这里没有这个人。$", '',
                     'clbNobody')
    ...
    create_trigger_t('clbAccept1',
                     "^(> )*贝海石在你的耳边悄声说道：我接到飞鸽传书，(龙灵堂|狮威堂|熊敏堂|虎猛堂|豹捷堂)属下帮众(\\D*)在(\\D*)处遇到袭击",
                     '', 'clbConsider')
    create_trigger_t('clbAccept2',
                     "^(> )*贝海石说道：「(你不是已经知道了|啊……，对不起|你不是已经进展|我不是告诉你了吗)",
                     '', 'clbFangqi')
    create_trigger_t('clbAccept3',
                     "^(> )*贝海石说道：「你刚做完长乐帮任务",
                     '', 'clbFail')
    create_trigger_t('clbAccept4',
                     "^(> )*贝海石说道：「(我帮现在比较空闲|现在我这里没有给你的任务|暂时没有任务需要做)",
                     '', 'clbBusy')
    create_trigger_t('clbAccept5',
                     "^(> )*贝海石说道：「你刚做完(惩恶扬善|大理送信)任务",
                     '', 'clbBusy')
    create_trigger_t('clbAccept6',
                     "^(> )*贝海石说道：「现在暂时没有适合你的工作",
                     '', 'clbBusy')
    create_trigger_t('clbAccept7',
                     "^(> )*贝海石说道：「暂时没有任务需要做，你过一会儿再来吧",
                     '', 'clbBusy')
    create_trigger_t('clbAccept8',
                     "^(> )*贝海石说道：「既然你无法完成", '',
                     'clbFail')
    create_trigger_t('clbAccept9',
                     "^(> )*贝海石说道：「你根本就没有领任务",
                     '', 'clbFail')
    ...
    create_trigger_t('clbChakan1',
                     '^(> )*看来只能查看到这步了，还是先回去交差吧。',
                     '', 'clbCkBack')
    create_trigger_t('clbChakan2', '^(> )*(\\D*)「啪」的一声倒在地上',
                     '', 'clbDie')
    -- create_trigger_t('clbChakan3','^(> )*你把 "action" 设定为 "chakan" 成功完成。$','','clbCkBack')
    create_trigger_t('clbChakan3',
                     '^(> )*你暗下寻思询问(\\D*)的(\\D*)\\((\\D*)\\)或许能得到提示，传闻(他|她)曾在(\\D*)附近出现。',
                     '', 'clbwho')
    ...
    create_trigger_t('clbFinish1',
                     '^(> )*贝海石说道：「(你根本就没有领任务|既然已经知道了是|哎，可惜不能手刃此奸贼|做的好！这些金子)',
                     '', 'clbFinish')
    create_trigger_t('clbFinish2',
                     "^(> )*贝海石说道：「你真的完成任务了", '',
                     'clbFangqi')
...
function clbGo() return go(clbaskjob, "长乐帮", "小厅") end
function clbaskjob()
    if newbie == 1 then
        return zhunbeineili(clbStart)
    else
        return clbStart()
    end
end
...
function clbConsider(n, l, w)
    ...
    if not job.room or not path_cal() then
        messageShow('长乐帮任务：任务地点【' .. job.where ..
                        '】不可到达，任务放弃。')
...
function clbFind()
    DeleteTriggerGroup("clbFind")
    create_trigger_t('clbFind1', '^(> )*\\D*' .. job.target .. '\\((\\D*)\\)',
                     '', 'clbTarget')
    create_trigger_t('clbFind2',
                     '^(> )*\\D*' .. job.target .. '的尸体\\((\\D*)\\)', '',
                     'clbTarget')
...
function clbFindAct()
    ...
    job.flag()
    exe('look')
    messageShow('长乐帮任务：开始寻找【' .. dest.area .. dest.room ..
                    '】的' .. '【' .. job.target .. '】！')
    return find()
end
...
function clbChakan()
    exe('yield yes')
    exe('halt')
    for i = 1, 3 do exe('get cu bu from corpse ' .. i) end
    exe('chakan cu bu')
    exe('chakan corpse')
...
function clbwho(n, l, w)
    messageShow("任务监控：长乐帮询问的玩家是【" .. w[2] ..
                    "】的【" .. w[3] .. "】，id【" .. w[4] ..
                    "】，位于【" .. w[6] .. "】", "deepskyblue")
    ...
    if not getAddr(job.where2) then
        ...
        messageShow('长乐帮任务⑵：开始前往【' .. job.where2 ..
                        '】寻找玩家！【' .. w[3] .. '】，id【' .. w[4] ..
                        '】')
...
function clbFindPlayer()
    ...
    DeleteTriggerGroup("clb_xunwen")
    create_trigger_t('clb_xunwen4', '^(> )*\\D*' .. job.playername .. '(\\D*)',
                     '', 'xWgood')
    create_trigger_t('clb_xunwen1',
                     '^(> )*\\D*在你的耳边悄声说道：适才(\\D*)弟子(\\D*)从我身边经过，好像前往了(\\D*)。',
                     '', 'clbConsider2')
    create_trigger_t('clb_xunwen2',
                     '^(> )*\\D*在你的耳边悄声说道：刚才是有人从我身边经过，可惜我没看清楚他往哪个方向离开了。',
                     '', 'clbBack')
    create_trigger_t('clb_xunwen3',
                     '^(> )*\\D*你不是已经询问过了？还不快去。',
                     '', 'clbBack')
    ...
    DeleteTriggerGroup("clb_debug")
    create_trigger_t('clb_debug1',
                     '^(> )*\\D*你(身旁有这个人么|不是已经询问过了)',
                     '', 'debugBack')
...
function xunWen() exe('xunwen ' .. job.playerid) end
...
function clbfight()
    DeleteTriggerGroup("clbFight")
    create_trigger_t('clbFight1', '^(> )*(\\D*)「啪」的一声倒在地上',
                     '', 'clbBack')
    create_trigger_t('clbFight2',
                     '^(> )*(\\D*)大喊一声：不好！！转身几个起落就不见了',
                     '', 'clbBack')
    create_trigger_t('clbFight3',
                     "^(> )*(\\D*)看见你手中握着的粗布碎片，不由脸色大变，大喝道：既然已被你看破，你也就别活了！",
                     '', 'clbKillAct')
    create_trigger_t('clbFight4',
                     "^(> )*(\\D*)大喊一声：老子不奉陪了！转身几个起落就不见了",
                     '', 'clbBack')
    create_trigger_t('clbFight5',
                     '^>*\\s*一股暖流发自丹田流向全身，慢慢地你又恢复了知觉……',
                     '', 'clbFangqiGo')
    create_trigger_t('clbFight6', "^(他|她)装备着：$", '', 'npcWeapon')
    create_trigger_t('clbFight7', '^(> )*你要对谁做这个动作？', '',
                     'clbFind')

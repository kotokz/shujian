...
function dmlTriggers()
    ...
    create_trigger_t('dmlXueAsk1',
                     '^(> )*你向薛慕华打听有关『疗伤』的消息。$',
                     '', 'dmlXueAccept')
    create_trigger_t('dmlXueAsk2', '^(> )*这里没有这个人', '',
                     'chedml_xue_fail')
    ...
    create_trigger_t('dmlXueAccept1',
                     '^(> )*薛慕华「嘿嘿嘿」奸笑了几声。$', '',
                     'dmlXueTeach')
    create_trigger_t('dmlXueAccept2',
                     '^(> )*一柱香的工夫过去了，你觉得伤势已经基本痊愈了。',
                     '', 'dmlXueHeal')
    create_trigger_t('dmlXueAccept3',
                     '^(> )*薛神医拿出一根银针轻轻捻入你受伤部位附近的穴道',
                     '', 'dmlXueWait')
    create_trigger_t('dmlXueAccept4',
                     '^(> )*薛慕华似乎不懂你的意思。$', '',
                     'dmlXueHeal')
    create_trigger_t('dmlXueAccept5',
    ...
    create_trigger_t('dmltopcheck3',
                     ...
                     '', 'dmlCheckToplistAct')
    create_trigger_t('dmltopcheck4',
                     '^>*\\S*\\s*还剩\\s*\\d*\\s*行\\s*==\\s*\\(ENTER\\s*继续下一页，q\\s*离开，b\\s*前一页\\)\\s*\\d*:*\\s*\\S*\\s*(\\d*)\\s*(\\D*)\\((\\D*)\\)\\s*\\D*\\s*(\\d*)\\s*\\S*\\s*\\S*$',
                     '', 'dmlCheckToplistAct')
    create_trigger_t('dmltopcheck5',
                     '^>*\\d*:*  你今天一共有五次挑战机会，现在还有\\D*次。',
                     '', 'dmlInfo')
    create_trigger_t('dmltopcheck6',
    ...
    create_trigger_t('dmlnofight1',
                     '^(> )*这里不准战斗，也不准打坐。', '',
                     'dmlNoFightRoom')
    create_trigger_t('dmlnofight2', '^(> )*您先歇口气再说话吧。', '',
                     'dmlAskAgain')
    for i = 1, 2 do
        SetTriggerOption("dmlnofight" .. i, "group", "dmlnofight")
    end
    DeleteTriggerGroup('dmlfight')
    create_trigger_t('dmlfight1',
                     '^(> )*突然间，一个身影出在漫天蝶影之中。你定睛一看，正是(\\D*) (\\D*)\\((\\D*)\\)。',
                     '', 'dmlFight')
    create_trigger_t('dmlfight2',
                     '^(> )*你挑战\\D*成功，获得(\\D*)点江湖声望和(\\D*)个竞技币。',
                     '', 'dmlSucceed')
    create_trigger_t('dmlfight3',
                     '^(> )*接着一道神光笼罩着你，你的内力有部分恢复！',
                     '', 'dmlCheck1')
    create_trigger_t('dmlfight4',
                     '^(> )*你怀疑自己也许从来就没有到过一个叫做梦蝶楼的地方。',
                     '', 'dmlOut')
    create_trigger_t('dmlfight5', '^(> )*你今天已经挑战过\\D*了。',
                     '', 'dmlCheckNextTarget')
    create_trigger_t('dmlfight6', '^(> )*你不能挑战自己！', '',
                     'dmlCheckNextTarget')
    create_trigger_t('dmlfight7',
                     '^(> )*没有发现一个id是\\D*的玩家。请注意在线玩家列表！',
                     '', 'dmlTakePlace')
    create_trigger_t('dmlfight8',
                     '^(> )*竞技时间已过，你无法向\\D*发起挑战。',
                     '', 'dmlOver')
    create_trigger_t('dmlfight9',
                     '^(> )*突然一道神光笼罩着你，你的精气神竟然全部恢复了！',
                     '', 'dmlFailed')
...
function dmlPkTriggers()
    ...
    create_triggerex_lvl('dmlPk1',
                         '^(> )*如果你要和(\\D*)\\((\\D*)\\)性命相搏，',
                         '', 'dmlPkAct', 99)
    create_triggerex_lvl('dmlPk2', '^>*\\s*加油！加油！', '', 'dmlInPk',
                         99)
    create_triggerex_lvl('dmlPk3', '^>*这里没有这个人。', '',
                         'dmlPkTargetNotFound', 99)
    create_trigger_t('dmlPk4', '^>*你今天的挑战次数已用完。$', '',
                     'dmlChanceOut')
    create_triggerex_lvl('dmlPk5', '^>*城隍庙\\s*-\\s*', '', 'dmlPkFail', 99)
    create_triggerex_lvl('dmlPk6', '^(> )*(\\D*)「啪」的一声倒在地上',
                         '', 'dmlPkSucceed', 99)
    create_triggerex_lvl('dmlPk7',
    ...
    create_trigger_t('dmldazuo1',
                     ...
                     '', 'dmlPkNeiliCheck')
    SetTriggerOption('dmldazuo1', 'group', 'dmldazuo')
    create_trigger_t('pkPfm1',
                     '^(> )*你把 "pfmpfm" 设定为 "\\D*;perform\\s*(\\D*)\\s*" 成功完成。',
                     '', 'pkPfmSet')
...
function dmlSettings()
    ...
    l_result = utils.inputbox(
                   ...
                   "初始挑战范围等级上限",
                   GetVariable("dmlIncreaseLvl"), "宋体", "12")
    ...
    l_result = utils.inputbox(
                   ...
                   "初始挑战范围等级下限",
                   GetVariable("dmlDecreaseLvl"), "宋体", "12")
    ...
    l_result = utils.inputbox(
                   ...
                   "备用列表挑战范围等级上限",
                   GetVariable("dmlNewIncreaseLvl"), "宋体", "12")
...
function dml_check()
    if not GetVariable('dmlMode') then
        ...
        if tonumber(GetVariable('dmlMode')) == 3 and
            (not GetVariable('dmlIncreaseLvl') or
                not GetVariable('dmlDecreaseLvl') or
                ...
                not GetVariable('dmlPkFlag')) then return dmlSettings() end
    end
    DeleteTimer('idle')
    dmlTriggers()
    exe('hp')
    print('蝶梦楼全自动挑战模块已启动.........')
    messageShow('蝶梦楼全自动模块：本日蝶梦楼备战中.........',
                'yellow', 'black')
...
function dmlNoFightRoom() return go(dmlNeili, '扬州城', '莲性寺白塔') end
function dmlEat()
    if locl.room == '茶亭' then
        exe(
            'sit chair;knock table;get tao;#3(eat tao);get cha;#3(drink cha);drop cha;drop tao;hp')
...
function dmlHeal1() exe('ask xue muhua about 疗伤') end
function dmlXueBusy() return check_busy(dmlXueOk, 0.4) end
function dmlXueOk()
...
function dmlNeili()
    if locl.room == '莲性寺白塔' then
        if condition.busy and condition.busy > 0 then return checkJob() end
        exe('yun qi;yun jing;yun jingli')
        if hp.neili < hp.neili_max * 1.5 then
            return dmlDazuo()
        else
            return checkWait(dekedml, 5)
        end
    else
...
function checkTop(func)
    ...
    create_trigger_t('checkTop1',
                     "^(> )*(你现在已经组合|你已准备有一种技能了|你至少不会这两种拳脚技能的其中之一)",
                     '', 'beiOk')
    create_trigger_t('checkTop2',
                     "^(> )*你现在没有激发任何有效特殊技能。",
                     '', 'beiNone')
...
function beiTimer() return create_timer_s('bei', 0.5, 'beiTimerSet') end
function beiTimerSet() exe('bei bei bei') end
function beiNone()
...
function beiOk()
    ...
    if beihook == nil then beihook = checkTopDone end
    return beihook()
end
...
function dmlCheckToplistAct(n, l, w)
    ...
    if string.find(score.id, targetId) then
        userRank = targetRank
        userLvl = targetLvl
        messageShow('玩家当前蝶梦楼【排名】' .. userRank ..
                        '位|【等级】' .. userLvl .. '级', 'gold')
    end
end
function dmlInfo()
    ...
    if tonumber(GetVariable('dmlMode')) == 1 then
        ...
        ColourNote('doggerblue', 'black', '挑战目标排名等级为',
                   'yellow', 'black', '【' .. _t1 .. '】', 'doggerblue',
                   'black', '级之上')
    ...
    elseif tonumber(GetVariable('dmlMode')) == 3 then
            ...
            if GetVariable('dmlFightOrder') == '上' then
                for i = 1, table.getn(_target) do
                    if _target[i].rank < userRank then
                        if _target[i].lvl > _t1 and _target[i].lvl < _t2 and
                            _target[i].id ~= score.id then
                            DeleteTimer('dmlWait')
                            table.insert(target, {
                                name = _target[i].name,
                                id = _target[i].id,
                        ...
                        if _target[i].lvl > _t1 - GetVariable('dmlDecreaseLvl') and
                            _target[i].lvl < _t2 + GetVariable('dmlIncreaseLvl') then
        ...
        ColourNote('doggerblue', 'black', '挑战目标排名等级为',
                   'yellow', 'black', '【' .. _t1 .. '】', 'doggerblue',
                   'black', '级到', 'yellow', 'black', '【' .. _t2 .. '】',
                   'doggerblue', 'black', '级之间')
    end
    ...
    if dmlCheckCnt < 3 then
        if table.getn(target) < 5 then
            if table.getn(backupList) < 5 then
                ...
                if tonumber(GetVariable('dmlMode')) == 1 or
                    GetVariable('dmlPerTargetTwice') == 'yes' then
                    ColourNote('lime', 'black',
                               '共查询到【' .. (table.getn(backupList) / 2) ..
                                   '】位符合条件的目标')
                    messageShow('蝶梦楼全自动模块：共查询到【' ..
                                    (table.getn(backupList) / 2) ..
                                    '】位符合条件的目标.........',
                                'lime', 'black')
                    for i = table.getn(backupList), (table.getn(backupList) - 4), -1 do
                        ColourNote('lime', 'black', '挑战目标名称',
                                   'white', 'black',
                                   '【' .. backupList[i].name .. '】', 'lime',
                                   'black', ' 目标ID', 'white', 'black',
                                   '【' .. backupList[i].id .. '】', 'lime',
                                   'black', ' 目标排名', 'white', 'black',
                                   '【' .. backupList[i].rank .. '】', 'lime',
                                   'black', '位 目标等级', 'white',
                                   'black', '【' .. backupList[i].lvl .. '】',
                                   'lime', 'black', '级。')
                    end
                else
                    ColourNote('lime', 'black', '共查询到【' ..
                                   table.getn(backupList) ..
                                   '】位符合条件的目标')
                    messageShow('蝶梦楼全自动模块：共查询到【' ..
                                    table.getn(backupList) ..
                                    '】位符合条件的目标.........',
            ...
            if tonumber(GetVariable('dmlMode')) == 1 or
                ...
                for i = table.getn(target), (table.getn(target) - 4), -1 do
                    ColourNote('lime', 'black', '挑战目标名称', 'white',
                               'black', '【' .. target[i].name .. '】',
                               'lime', 'black', ' 目标ID', 'white', 'black',
                               '【' .. target[i].id .. '】', 'lime', 'black',
                               ' 目标排名', 'white', 'black',
                               '【' .. target[i].rank .. '】', 'lime',
                               'black', '位 目标等级', 'white', 'black',
                               '【' .. target[i].lvl .. '】', 'lime', 'black',
                               '级。')
                end
            else
                ColourNote('lime', 'black', '共查询到【' ..
...
function dmlSucceed(n, l, w)
    ...
    local _cnt = l_cnt + 1
    if table.getn(target) >= 5 then
        messageShow('挑战【' .. target[_cnt].name .. '】 【' ..
                        target[_cnt].id .. '】成功！', 'lime', 'black')
        table.insert(pkList, {name = target[_cnt].name, id = target[_cnt].id})
    else
        messageShow('挑战【' .. backupList[_cnt].name .. '】 【' ..
                        backupList[_cnt].id .. '】成功！', 'lime', 'black')
        table.insert(pkList,
                     {name = backupList[_cnt].name, id = backupList[_cnt].id})
    end
...
function dmlFailed()
    ...
    if table.getn(target) >= 5 then
        s = target[_cnt].name .. '|'
        messageShow('挑战【' .. target[_cnt].name .. '】 【' ..
                        target[_cnt].id .. '】失败！', 'fuchsia', 'black')
    else
        s = backupList[_cnt].name .. '|'
        messageShow('挑战【' .. backupList[_cnt].name .. '】 【' ..
                        backupList[_cnt].id .. '】失败！', 'fuchsia',
                    'black')
    end
    f:write(s)
    f:close()
...
function dmlAskOk() EnableTriggerGroup('dmlask', false) end
function dmlAskAgain() return checkTop(dmlAsk) end
function dmlAsk()
    ...
    if dmlFightCnt < 5 then
        if l_cnt > 0 then
            if table.getn(target) >= 5 then
                messageShow(
                    '蝶梦楼全自动模块：现在开始挑战【' ..
                        target[l_cnt].name .. '】 【' .. target[l_cnt].id ..
                        '】.........', 'yellow')
                return exe('dream ' .. target[l_cnt].id)
            else
                messageShow(
                    '蝶梦楼全自动模块：现在开始挑战【' ..
                        backupList[l_cnt].name .. '】 【' ..
                        backupList[l_cnt].id .. '】.........', 'yellow')
...
function dmlTopTake()
    if table.getn(target) >= 5 then
        if target[l_cnt].rank < userRank then
            ...
            print('目标【' .. target[l_cnt].name ..
                      '】离线，取代对方排名【' .. target[l_cnt].rank ..
                      '】位')
            messageShow('蝶梦楼全自动模块：挑战目标【' ..
                            target[l_cnt].name ..
                            '】离线，取代对方排名至【' ..
                            target[l_cnt].rank .. '】位.........', 'lime')
            table.insert(blackList, target[l_cnt].id)
        end
    else
        if backupList[l_cnt].rank < userRank then
            ...
            print('目标【' .. blackList[l_cnt].name ..
                      '】离线，取代对方排名【' ..
                      blackList[l_cnt].rank .. '】位')
            messageShow('蝶梦楼全自动模块：挑战目标【' ..
                            blackList[l_cnt].name ..
                            '】离线，取代对方排名至【' ..
                            blackList[l_cnt].rank .. '】位.........', 'lime')
            table.insert(blackList, backupList[l_cnt].id)
        end
    end
    l_cnt = l_cnt - 1
...
function dmlFight(n, l, w)
    ...
    local fn = GetInfo(67) .. "logs\\" .. score.id .. '蝶梦楼竞技记录' ..
                   dmlFightCnt .. ".log"
    OpenLog(fn, false)
    ColourNote('Lime', 'black', '开始写入蝶梦楼竞技记录到' .. fn ..
                   '，写入方式为擦除重写！')
    l_cnt = l_cnt - 1
    job.name = 'diemenglou'
    npc_name = ''
...
function dmlPkCheck() exe('kill ' .. npc_id) end
...
function hpHeqiCheck(n, l, w)
    ...
    if hp.qixue_per < 70 and cty_cur > 0 then exe('fu chantui yao') end
...
function dmlPkSucceed()
    messageShow('蝶梦楼全自动模块：PK玩家【' .. npc_name ..
                    '】成功.........', 'lime')

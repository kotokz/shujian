...
function opendummy()
    ...
    create_triggerex_lvl('mydummy1', '^(> )*(\\! )*【' .. score.party ..
                             '】\\D*\\(\\N*\\)：听说有个(\\D*)假冒(\\D*)(容|派|帮|寺|岛|教|0)(\\D*)\\((\\D*)\\)之名，在(\\D*)一带',
                         '', 'trundummy', 97)
...
function dummyfind()
    if job.where4 ~= nil then
        if getAddr(job.where4) ~= false then
            ...
            messageShow('杀大米任务：任务地点【' .. job.where4 ..
                            '】不可到达，任务放弃。', 'red')
            return check_bei(dummyover)
        end
    else
        messageShow(
            '杀大米任务：任务地点出错，【' .. job.where4 ..
                '】不存在，任务放弃。', 'red')
        return check_bei(dummyover)
    end
...
function dummy_find_go()
    DeleteTriggerGroup("dummyfind")
    create_trigger_t('dummyfind1',
                     '^>*\\s*\\D*' .. job.target4 .. '\\((\\D*)\\)', '',
                     'dummyco')
    create_trigger_t('dummyfind2', '^(> )*(他|她)的(武艺|武功)看', '',
                     'dummylevel')
    create_trigger_t('dummyfind3',
                     "^  □(竹棒|长剑|长鞭|一块铁令|钢刀|箫)\\(",
                     '', 'xueshan_judge_weapon')
...
function dummy_find_act()
    EnableTriggerGroup("dummyfind", true)
    if score.party == '神龙教' then
        map.rooms["sld/lgxroom"].ways["#outSld"] = nil
    end
    job.flag()
    exe('look')
    messageShow('大米任务：大米【' .. job.target4 .. '】在【' ..
                    job.room .. '】一带出现，开始寻找！',
                'greenyellow')
    return find()
end
...
function dummylevel(n, l, w)
    job.level = w[3]
    if string.find(job.level, "武艺") then return dummy_judge() end
    messageShow('打不过【' .. job.target4 ..
                    '】的大米，放弃任务！', 'red')
    return dummyover()
end
function dummykill()
    DeleteTriggerGroup('dummykill')
    create_trigger_t('dummy1kill',
                     '^>*\\s*(\\D*)「啪」的一声倒在地上', '',
                     'dummyover')
    create_trigger_t('dummy2kill',
                     '^>*\\s*(\\D*)神志迷糊，脚下一个不稳，倒在地上昏了过去。',
                     '', 'dummy_kill_faint')
    create_trigger_t('dummy3kill', '^(> )*这里不准战斗。', '',
                     'dummy_kill_nofight')
    ...
    kezhiwugong()
    kezhiwugongAddTarget(job.target4, job.id3)
    messageShow('杀大米：开始杀大米：【' .. job.target4 .. '】！',
                'greenyellow')
...
function dummy_judge(n, l, w)
    ...
    job.skill = '无'
    messageShow('杀大米：大米来自【' .. job.party2 .. '】！',
                'greenyellow')

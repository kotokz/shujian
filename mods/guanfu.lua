...
function guanfu_choice()
    ...
    create_trigger_t('guanfuAccept1',
                     "^(> )*你分开行人来到近前，目光淡淡的扫视了几眼告示，嘴角微微一翘，神态自如的把通缉(\\D*)的文书揭了下来。",
                     '', 'guanfuNpc')
    create_trigger_t('guanfuAccept2',
                     '^\\s*(就地格杀|缉拿归案)\\s*(\\D*)\\((\\D*)\\)\\s*武功：(\\D*)\\s*等级：\\s*(\\d*)\\s*赏金：(\\D*)',
                     '', 'guanfu_checklist')
...
function guanfu_checklist(n, l, w)
    ...
    if GF_target.Way == "缉拿归案" then
        if GF_target.Lvl < hp.pot_max - 300 or GF_target.Lvl > hp.pot_max - 200 then
            return
        end
        table.insert(_GF_target1, {
            way = GF_target.Way,
            name = GF_target.Name,
            id = GF_target.Id,
            power = GF_target.Power,
            lvl = GF_target.Lvl,
            gold = GF_target.Gold
        })
    else
        if GF_target.Lvl < hp.pot_max - 200 or GF_target.Lvl > hp.pot_max - 100 then
            return
        end
        table.insert(_GF_target2, {
            way = GF_target.Way,
            name = GF_target.Name,
            id = GF_target.Id,
            power = GF_target.Power,
...
function gftest()
    ...
    for i = 1, table.getn(_GF_target1) do
        print(_GF_target1[i].way, _GF_target1[i].name, _GF_target1[i].id,
              _GF_target1[i].power, _GF_target1[i].lvl, _GF_target1[i].gold)
    end
    table.sort(_GF_target2, function(a, b)
        if a.lvl ~= b.lvl then return a.lvl < b.lvl end
    end)
    for i = 1, table.getn(_GF_target2) do
        print(_GF_target2[i].way, _GF_target2[i].name, _GF_target2[i].id,
              _GF_target2[i].power, _GF_target2[i].lvl, _GF_target2[i].gold)
    end
end
function guanfuNpc()
    DeleteTriggerGroup("guanfu_npc")
    create_trigger_t('guanfuNpc2', '\\s*\\D*最后一次出现在(\\D*)附近',
                     '', 'guanfuConsider')
...
function check_wenshu() exe('l wenshu') end
...
function guanfuConsider(n, l, w)
    ...
    if not job.room or not path_cal() then
        messageShow('官府任务：任务地点【' .. job.where ..
                        '】不可到达，任务放弃。', "Plum")
        exe('drop wenshu')
        return checkPrepare()
    end
...
function guanfu_FindAct()
    ...
    exe('look')
    find()
    messageShow('官府任务：已到达任务地点【' .. job.area ..
                    '】，开始寻找【' .. job.target .. '】', 'lime',
                'black')
end
...
function guanfuFindAgain() return go(guanfu_FindAct, dest.area, dest.room) end
function guanfuFindFail()
    messageShow('官府任务：搜索丢失【' .. job.target ..
                    '】三次！回去放弃！', 'white', 'black')

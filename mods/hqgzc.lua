...
function hqgzcTrigger()
	...
	create_trigger_t('hqgzcAskFail1',"^(> )*洪七公说道：「既然做不了，也就不勉强你了。」",'','hqgzc')
	...
	create_trigger_t('hqgzcAccept3',"^(> )*洪七公说道：「我现在不饿，你还是先去休息一会吧。」",'','hqgzcBusy')
	...
	create_trigger_t('hqgzcAccept7',"^(> )*(洪七公说道：「\\D*，你又没在我这里领任务，瞎放弃什么呀|洪七公说道：「我有叫你去做什么吗？你放弃什么啊？」)",'','hqgzcStart')
	...
	create_trigger_t('hqgzcFinish6','^(> )*洪七公说道：「今天我还是先指点你武功吧。」','','hqgzcFinishGold1')
	...
	for j=1,4 do
		...
		do
			if cooking.flag[i]==j then
				...
				create_trigger_t('hqgzcTrade7',"^(> )*你没有那么多的白银。",'','hqgzcPay2')
				...
				--exe('set no_kill_ap')
				--pfmjineng()
				messageShow('做菜任务：前往任务地点【'..job.where..'】，准备寻找【'..dest.area .. dest.room ..'】的'..'【'.. job.target ..'】！','white','black')
...
function hqgzcTradeOK()
	...
	exe('follow none')
	--flag.idle = 0
	messageShow('做菜任务：任务找到目标【'..job.target..'】地点【'..job.where..'】，取得做菜材料【'..job.menu..'】。','lime','black')
	for key,value in pairs(cooking.target)
	do

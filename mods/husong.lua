...
function husongTrigger()
    ...
    create_trigger_t('husongAsk1', "^(> )*你向玄慈大师打听有关", '',
                     'husongAsk')
    create_trigger_t('husongAsk2', "^(> )*这里没有这个人。$", '',
                     'husongNobody')
    ...
    create_trigger_t('husongAccept1',
                     "^(> )*玄慈大师说道：「(\\D*)现在(\\D*)。」$",
                     '', 'husongConsider')
    create_trigger_t('husongAccept2',
                     "^(> )*玄慈大师说道：「(\\D*)似乎尚无力单独完成这个任务。」$",
                     '', 'husongFail')
    create_trigger_t('husongAccept3',
                     "^(> )*玄慈大师说道：「嗯，已经有人在帮我了，你还是去忙点别的什么吧。」",
                     '', 'husongBusy')
    create_trigger_t('husongAccept4',
                     "^(> )*玄慈大师说道：「你刚才不是已经问过了吗？」",
                     '', 'husongQuit')
    create_trigger_t('husongAccept5',
                     "^(> )*玄慈大师说道：「你武功精进也太慢了，老衲怎么放心让你去干啊。」",
                     '', 'husongBusy')
    create_triggerex_lvl('husongAccept6',
                         "^(> )*玄慈大师说道：「我这里现在没有什么任务给你。」",
                         '', 'husongBusy', 95)
    ...
    create_trigger_t('husongDashiAsk1', "^(> )*你向(\\D*)大师打听有关",
                     '', 'husongDashiStart')
    create_trigger_t('husongDashiAsk2', "^(> )*这里没有这个人。$", '',
                     'husongNobody')
    ...
    create_trigger_t('husongJiaotu1',
                     '^(> )*突然从路边冲出一个魔教教徒，一声不吭的向(\\D*)冲去。',
                     '', 'husongJiaotu')
    create_trigger_t('husongJiaotu2',
                     '^(> )*看起来魔教教徒想杀死你！', '',
                     'husongFight')
    ...
    create_trigger_t('husongFight1',
                     "^(> )*" .. score.id .. "'s jiaotu 不在这里", '',
                     'husongFightOver')
    create_trigger_t('husongFight2',
                     '^(> )*(\\D*)「啪」的一声倒在地上', '',
                     'husongDie')
    create_trigger_t('husongFight3',
                     '^(> )*你仔细的上前打量了魔教教徒一番。',
                     '', 'husongCompare')
    ...
    create_trigger_t('husongFinish1',
                     '^(> )*(\\D*)不幸阵亡，你任务失败。', '',
                     'husongDashiDie')
    create_trigger_t('husongFinish2',
                     "^(> )*好，任务完成，你得到了(\\D*)点实战经验和(\\D*)点潜能。",
                     '', 'husongFinishExp')
    create_trigger_t('husongFinish3',
                     '^(> )*(\\D*)在你的耳边悄声说道：刚才我碰到方生师弟，他似乎有一些',
                     '', 'husongFinishQuest')
...
function husongGo() return go(husongStart, "嵩山少林", "方丈室") end
...
function husongjinhe()
    local l_flag = false
    for p in pairs(Bag) do if weaponStore[p] then l_flag = true end end
    if l_flag then return husongCunWeapon() end
    if Bag["锦盒"] then exe('cun jin he') end
    checkBags()
    return check_busy(husongCunBook)
end
function husongCunBook()
    if Bag["锦盒"] then return husongCunJinhe() end
...
function husongCunHammer()
    ...
    if Bag["韦兰之锤"] then exe("cun weilan's hammer") end
    checkBags()
    return check_busy(husongCunQuit)
end
function husongCunQuit()
    if Bag["韦兰之锤"] then return husongCunHammer() end
    flag.quit = true
    wait.make(function()
        exe('quit')
        wait.time(2)
...
function husongFinishExp(n, l, w)
    ...
    messageShow('少林护送奖励：干掉【' .. tmp.jiaotu ..
                    '】个魔教教徒，获得经验:【' .. w[2] ..
                    '】点，，潜能:【' .. w[3] .. '】点！',
                'darkorange')
    return husongFinish()
...
function husongQuest() return go(husongQuestAsk, "嵩山少林", "心禅堂") end
...
function husongConsider(n, l, w)
    ...
    if not job.room or not path_cal() then
        messageShow('少林护送任务：任务地点【' .. job.where ..
                        '】不可到达，任务放弃。')
...
function husongDashi()
    if locl.room ~= job.room then return go(husongDashi, job.area, job.room) end
    if not locl.id[job.target] then return husongBusy() end
...
function husongStep()
    flag.idle = nil
    EnableTriggerGroup("husongFinish", true)
    if locl.room == "白云庵" then return end
    if type(road.husong) ~= "table" then return husongWhere() end
    if table.getn(road.husong) == 0 then return husongWhere() end
    if hp.qixue_per < 60 then exe('fu dan') end
    ...
    flag.jiaotu = nil
    flag.lost = nil
    if locl.room == "山门殿" then exe('open gate') end
    if locl.room == "渡船" then return husongBoat() end
...
function husongBoatWait()
    flag.idle = nil
    DeleteTriggerGroup("husongBoat")
    create_trigger_t('husongBoat1',
                     '^>*\\s*(说着将一块踏脚板搭上堤岸，形成一个出去|渡船猛地一震，已经靠岸)',
                     '', 'husongBoatOut')
...
function husongLocate()
    DeleteTimer('husong')
    for i = 1, 2 do exe('get silver from corpse ' .. i) end
    if flag.jiaotu then return end
    locate()
...
function husongChkDashi()
    ...
    -- end
    flag.jiaotu = nil
    if isNil(road.id) then return husongWhere() end
    if locl.room ~= map.rooms[road.id].name then return husongWhere() end
...
function husongWhere()
    ...
    elseif locl.room == "陕晋渡口" then
        messageShow('少林护送任务：在【' .. locl.area .. locl.room ..
                        '】找到大师，重新启程。')
        ...
        if table.getn(l_rooms) == 0 then
            ...
            if l_dest then
                road.id = l_dest
                messageShow('少林护送任务：在【' .. locl.area ..
                                locl.room .. '】找到大师，重新启程。')
                return husongDashiStart()
            else
                -- messageShow('少林护送任务：无法在地点【'.. locl.area .. locl.room ..'】重新启程，任务放弃。')
...
function husongDashiDie(n, l, w)
    if w[2] == job.target then return husongLostEver() end
end
function husongLostEver()
    dis_all()
    nobusy = 0
    messageShow(
        '少林护送任务：在地点【' .. locl.area .. locl.room ..
            '】永远的失去大师，任务放弃。', 'blue')
    return husongBusy()
end
function husongLost()
    DeleteTriggerGroup("husongFind")
    create_trigger_t('husongFind1',
                     '^(> )*\\D*' .. job.target .. '\\((\\D*)\\)$', '',
                     'husongTarget')
    create_trigger_t('husongFind2',
                     '^(> )*\\D*' .. job.target .. '正在运功疗伤', '',
                     'husongTarget')
    create_trigger_t('husongFind3',
                     '^(> )*\\D*' .. job.target .. '的尸体\\((\\D*)\\)', '',
                     'husongLostEver')
    create_trigger_t('husongFind4', '^(> )*\\D*' .. job.target ..
                         '\\((\\D*)\\) <昏迷不醒>$', '', 'husongTargetWait')
    create_trigger_t('husongFind5',
                     '^(> )*\\D*' .. job.target .. '\\((\\D*)\\) <战斗中>$',
                     '', 'husongTarget')
...
function husongTargetWait()
    ...
    create_trigger_t('husongFaint1', '^(> )*\\D*' .. job.target ..
                         '身子动了动，口中呻吟了几声，清醒过来',
                     '', 'husongWake')

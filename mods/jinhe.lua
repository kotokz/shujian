...
function jinhe_get(n, l, w)
    ...
    wait.make(
        function()
            ...
            if #rooms < 1 then
                Log:error("锦盒房间不可到达或者找不到:" .. jinhe_location)
                return jinheOver(true)
            end

            Log:info("锦盒模块：开始挖宝，地点：" .. jinhe_location)
            ...
            Log:error("遍历完全部房间,在" .. jinhe_location .. "找不到锦盒")
            return jinheOver()
        end
    )
end

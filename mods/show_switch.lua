        ...
        if hotspot_id == 'btn_taoyuan' then
            if intaoyuan == 1 then
                ...
                map.rooms["dali/yideng/pubu"].ways["#yuRen"] =
                    "dali/yideng/shandong" -- 桃园县地图加入开关
            end
            if flags == 16 then
...
get_lost_locate = function(n, l, w)
    ...
    WindowRectOp(win3, miniwin.rect_fill, 0, 0, 300, 20,
                 ColourNameToRGB("black")) -- raised, filled, softer, flat 0x909090
    WindowFont(win3, "f", "新宋体", 10, true, false, false, false)
    WindowText(win3, "f", "失落的信笺地点：" .. lostletter_locate, 0, 0,
               300, 20, ColourNameToRGB("gold"), false) -- not Unicode
    WindowShow(win3, true) -- show it
...
function goll()
    if not getAddr(lostletter_locate) then
        ...
        messageShow('失落的信笺任务：前往【' .. lostletter_locate ..
                        '】开始寻找玩家！')
        ...
        if string.find(ll.area, '明教') or string.find(ll.area, '回疆') or
            string.find(ll.area, '天山') or string.find(ll.area, '伊犁') or
            string.find(ll.area, '大草原') or string.find(ll.area, '星宿') or
            string.find(ll.area, '昆仑') then
            return go_to('兰州大渡口')
        end
        if string.find(ll.area, '塘沽') or string.find(ll.area, '沧州') then
            return go_to('黄河流域大渡口')
        end
        if string.find(ll.area, '恒山') or string.find(ll.area, '黑木崖') or
            string.find(ll.area, '平定州') then
            return go_to('陕晋渡口')
        end
...
function goll_always()
    if not getAddr(lostletter_locate) then
        ...
        messageShow('失落的信笺任务：开始前往【' ..
                        lostletter_locate .. '】寻找玩家！')
...
function show_skills(n, l, w)
    if w[2] ~= '你' then EnableTrigger("hp7", false) end
    wait.make(function()
        wait.time(2)
...
function draw_statuswindow()
    ...
    local head_width = (WINDOW_WIDTH -
                           WindowTextWidth(win_status, "f1", "状态信息")) /
                           2
    ...
    if win_status_show == 0 then
        ...
        WindowText(win_status, "f5", "状态信息", -- text
        5, 9, 0, 0, -- rectangle
    ...
    if hp.jingxue_max ~= nil and hp.jingxue ~= nil then
        ...
        if hurtj > 95 then
            jing = "精血:" .. string.rep("■", 10) .. jingxue .. '/' ..
                       maxjing
        elseif hurtj > 49 then
            jing = "精血:" .. string.rep("■", rj) .. "" ..
                       string.rep("□", 10 - rj) .. jingxue .. '/' .. maxjing
        elseif hurtj > 9 then
            jing = "精血:" .. string.rep("■", rj) .. "" ..
                       string.rep("□", 10 - rj) .. jingxue .. '/' .. maxjing
        else
            jing = "精血:" .. string.rep("■", 1) .. "" ..
    ...
    if hp.jingli_max ~= nil and hp.jingli ~= nil then
        ...
        if hurtjingli > 95 then
            jl = "精力:" .. string.rep("■", 10) .. jingli .. '/' ..
                     maxjingli
        elseif hurtjingli > 49 then
            jl = "精力:" .. string.rep("■", rjl) .. "" ..
    ...
    if score.gold ~= nil then mygold = "存款:" .. score.gold end
    ...
    if score.xiangyun ~= nil then lixiang = "理相:" .. score.xiangyun end
    ...
    if score.sw ~= nil then sw = "声望:" .. score.sw end
    WindowText(win_status, "f1", sw, 5, 90, 0, 0, -- rectangle
    ColourNameToRGB("cyan"), -- colour
    false)

    local expp = "经验:" .. hp.exp
    if hp.exp ~= nil then expp = "经验:" .. hp.exp end
    ...
    if score.tb ~= nil then tbb = "通宝:" .. score.tb end
    WindowText(win_status, "f1", tbb, 5, 105, 0, 0, -- rectangle
    ColourNameToRGB("cyan"), -- colour
    false)

    local ybb = "元宝:"
    if score.yb ~= nil then ybb = "元宝:" .. score.yb end
    WindowText(win_status, "f1", ybb, 220, 105, 0, 0, -- rectangle
    ColourNameToRGB("cyan"), -- colour
    false)

    local jjbb = "竞技币:"
    if score.jjb ~= nil then jjbb = "竞技币:" .. score.jjb end
    ...
    local swww = "死亡:"
    if score.sww ~= nil then swww = "死亡:" .. score.sww end
    ...
    local zt = "状态:"
    if score.zt ~= nil then zt = "状态:" .. score.zt end
    WindowText(win_status, "f1", zt, 5, 150, 0, 0, -- rectangle
    ColourNameToRGB("cyan"), -- colour
    false)
    local loc = "定位:"
    if score.loc ~= nil then loc = "定位:" .. score.loc end
...
function draw_skillswindow()
    ...
    local head_width = (WINDOW_WIDTH -
                           WindowTextWidth(win_skills, "f1", "技能列表")) /
                           2
    ...
    if win_skills_show == 0 then
        ...
        WindowText(win_skills, "f5", "技能列表", -- text
        5, 9, 0, 0, -- rectangle
        ColourNameToRGB("yellow"), -- colour
        false) -- not Unicode
    else
        ...
        WindowText(win_skills, "f1", "共" .. #_basic ..
                       "种技能  当前人物等级：" .. hp.pot_max - 100, -- text
                   11, 30, 0, 0, -- rectangle
   ...
   WindowText (win_skills, "f1", 
        ...
        for i = 1, #my_misc_skills do -- for i = 1,#_basic do
            ...
            false) -- not Unicode
            if _skills_ch[i] ~= "数据丢失" and _skills_pot[i] ~=
                "数据丢失" and _skills_lev[i] ~= "数据丢失" then
...
function draw_bagwindow()
    ...
    -- ensure window visible
    local head_width = (WINDOW_WIDTH -
                           WindowTextWidth(win_bag, "f1", "物品列表")) / 2
    if win_bag_show == 0 then
        ...
        WindowText(win_bag, "f5", "物品列表", -- text
        5, 9, 0, 0, -- rectangle
        ColourNameToRGB("yellow"), -- colour
        false) -- not Unicode
    else
        ...
        WindowText(win_bag, "f1", "共" .. #_bag_ch .. "件物品", -- text
        5, 30, 0, 0, -- rectangle
        ColourNameToRGB("white"), -- colour
        false) -- not Unicode
        if Bag["ENCB"].value then
            WindowText(win_bag, "f1", "负重" .. Bag["ENCB"].value .. "%", -- text
...
function draw_beinangwindow()
    ...
    -- ensure window visible
    local head_width = (WINDOW_WIDTH -
                           WindowTextWidth(win_beinang, "f1", "背囊列表")) /
                           2
    if win_beinang_show == 0 then
        ...
        WindowText(win_beinang, "f5", "背囊列表", -- text
        5, 9, 0, 0, -- rectangle
        ColourNameToRGB("yellow"), -- colour
        false) -- not Unicode
    else

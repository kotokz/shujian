...
function window:init(height, width, refresh)
    ...
    local pop_menu = function(win_handle)
        local result = WindowMenu(win_handle, WindowInfo(win_handle, 14),
                                  WindowInfo(win_handle, 15),
                                  "^书剑Quest|-|隐藏|-|100*300|200*300")
        -- print(result)
        if result ~= "" then
            if result == "隐藏" then
                -- print("hide")
    ...
    local exist = false
    if windows then
        for _, v in ipairs(windows) do if v == win then exist = true end end
    end -- if any
    ...
    local windowinfo = movewindow.install(win, miniwin.pos_top_right, 0, true,
                                          nil, pop_menu) -- get position (default to right/middle)
    if exist == false or refresh == true then
        print("创建窗体!!")
        WindowCreate(win, windowinfo.window_left, windowinfo.window_top, width,
                     height, windowinfo.window_mode, windowinfo.window_flags,
    ...
    check(WindowFont(win, self.font_id, self.font_name, self.font_size, false,
                     false, false, false))
    WindowRectOp(win, miniwin.rect_fill, 0, 0, width, height,
                 self.background_colour) -- raised, filled, softer, flat
...
function window:DoGauge(sPrompt, Percent, sGoodColor, sMediumColor, sBadColor,
                        sText, bTextOnly)
...
function window:drawFrame(sPrompt)
    if (sPrompt ~= nil) then
        ...
        WindowText(self.name, self.font_id, sPrompt,
                   self.gauge_left - width + 5, self.vertical, 0, 0, 0x000000)

        -- frame rectangle
        WindowRectOp(self.name, 1, self.gauge_left + 5, self.vertical,
...
function window:drawGauge(sPrompt, Percent, sGoodColor, sMediumColor, sBadColor)
    ...
    -- fill rectangle
    WindowRectOp(self.name, 2, self.gauge_left + 6, self.vertical + 1,
                 self.gauge_left + pixels,
                 self.vertical + self.gauge_height - 1, ColourNameToRGB(colour))
end

-- 显示文字
function window:drawText(sText)
    if sText ~= nil then
        WindowText(self.name, self.font_id, sText, self.gauge_left,
                   self.vertical, 0, 0, 0x00FFFF)
        -- 计算下一个状态条的位置
        self.vertical = self.vertical + self.font_height
    end
    Redraw()
...
function window:draw_bar()
    ...
    for _, i in ipairs(self.items) do
        ...
        elseif i.Percent ~= nil then
            self:DoGauge(i.sPrompt, i.Percent, i.sGoodColor, i.sMediumColor,
                         i.sBadColor, i.sText, i.bTextOnly)
        end
    end
    Redraw()
end -- draw_bar
...
-- 添加进度条
function window:addGauge(id, sPrompt, Percent, sGoodColor, sMediumColor,
                         sBadColor, sText, bTextOnly)
    local item = {}
    for _, i in ipairs(self.items) do
        if i.id == id then
            item = i
...
-- 关闭窗口
function window:close() WindowDelete(self.name) end

function close_byName(win_name) WindowDelete(win_name) end

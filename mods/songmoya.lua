...
local falsesk = {["玄阴剑法"] = true}
local fearsk = {["玄阴剑法"] = true}
local Yptdangersk = {
    ["日月鞭法"] = true,
    ["打狗棒法"] = true,
    ["独孤九剑"] = true,
...
SMY_ID =
    { -- 这里出现的id，死衰不下山，而是降低组数，其它id死衰直接下山。
        ["wudi"] = true,
        ["yuyan"] = true,
        ["wuji"] = true
    }
...
function gbluSmyWait()
    ...
    SetTriggerOption("yptdz4", "group", "yptdz")
    SetTriggerOption("yptdz5", "group", "yptdz")
...
function ypt_check_skills(n, l, w)
    if sxjob.killer1 == tostring(w[1]) then
        ...
        if skillsjineng2[sxjob.skills1] then
            local l_cmd = GetVariable("smy_pqi")
            if skills["dugu-jiujian"] then
                l_cmd = setpo .. ';' .. l_cmd
            end
            create_alias('yptpfm1', 'yptpfm1',
        ...
        if skillsjineng3[sxjob.skills1] then
            local l_cmd = GetVariable("smy_pgang")
            if skills["dugu-jiujian"] then
                l_cmd = setpo .. ';' .. l_cmd
            end
            create_alias('yptpfm1', 'yptpfm1',
        ...
        if skillsjineng4[sxjob.skills1] then
            local l_cmd = GetVariable("smy_prou")
            if skills["dugu-jiujian"] then
                l_cmd = setpo .. ';' .. l_cmd
            end
            create_alias('yptpfm1', 'yptpfm1',
        ...
        if skillsjineng5[sxjob.skills1] then
            local l_cmd = GetVariable("smy_pkuai")
            if skills["dugu-jiujian"] then
                l_cmd = setpo .. ';' .. l_cmd
            end
            create_alias('yptpfm1', 'yptpfm1',
        ...
        if skillsjineng6[sxjob.skills1] then
            local l_cmd = GetVariable("smy_pman")
            if skills["dugu-jiujian"] then
                l_cmd = setpo .. ';' .. l_cmd
            end
            create_alias('yptpfm1', 'yptpfm1',
        ...
        if skillsjineng7[sxjob.skills1] then
            local l_cmd = GetVariable("smy_pmiao")
            if skills["dugu-jiujian"] then
                l_cmd = setpo .. ';' .. l_cmd
            end
            create_alias('yptpfm1', 'yptpfm1',
        ...
        if skillsjineng8[sxjob.skills1] then
            local l_cmd = GetVariable("smy_pxian")
            if skills["dugu-jiujian"] then
                l_cmd = setpo .. ';' .. l_cmd
            end
            create_alias('yptpfm1', 'yptpfm1',
        ...
        if skillsjineng11[sxjob.skills1] then
            local l_cmd = GetVariable("smy_pfmwu")
            if skills["dugu-jiujian"] then
                l_cmd = setpo .. ';' .. l_cmd
            end
            create_alias('yptpfm1', 'yptpfm1',
...
function yptKillgo()
    ...
    --[[if yptteam >= 7 then
           exe('jiali max;perform sword.qimen;set wimpycmd hp;set wimpy 100')
           else

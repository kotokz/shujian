...
function songxin_trigger()
	...
	create_trigger_t('songxin_accept7',"^>*\\s*褚万里\\D*说道：「\\D*你并没有领了送信的任务，说什么放弃啊",'','songxin_fangqi_heal')
...
function songxin_consider(n,l,w)
	...
	if job.where=="武当山院门" then
	   messageShow('送信任务：任务目标【'..job.target..'任务地点【'..job.where..'】容易出错，任务放弃。','blue')
       return check_halt(songxin_fangqi)
    end
...
function songxin_find()
    DeleteTriggerGroup("songxin_find")
    create_trigger_t('songxin_find1','^>*\\D*'..job.target..'\\((\\D*)\\)','','songxin_send')
...
function songxin_find_begin()
    ...
    create_trigger_t('songxin_begin1','^>*\\D*'..job.target..'\\((\\D*)\\)','','sx1job_wait')
...
function songxin2_find_begin()
    DeleteTriggerGroup("songxin_begin")
    create_trigger_t('songxin_begin1','^>*\\s*\\D*\\s*'..job.target..'\\((\\D*)\\)','','songxin2_wait')
    SetTriggerOption("songxin_begin1","group","songxin_begin")
    EnableTriggerGroup("songxin_begin",false)

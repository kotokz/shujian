...
function songxin_trigger()
    ...
    create_trigger_t('songxin_ask1', "^>*\\s*你向褚万里打听", '',
                     'songxin_ask')
    create_trigger_t('songxin_ask2', "^>*\\s*这里没有这个人。$", '',
                     'songxin_nobody')
    ...
    create_trigger_t('songxin_accept1',
                     "^>*\\s*褚万里\\D*说道：「\\D*，你不是本王府随从，此话从何说起？",
                     '', 'songxin_join')
    create_trigger_t('songxin_accept2',
                     "^>*\\s*褚万里\\D*说道：「(你先去休息一会吧！|现在暂时没有给你的任务，)",
                     '', 'songxin_busy')
    create_trigger_t('songxin_accept3',
                     "^>*\\s*褚万里\\D*说道：「既然\\D*做不了，也就罢了。",
                     '', 'songxin_fangqi_heal')
    create_trigger_t('songxin_accept4',
                     "^>*\\s*褚万里\\D*说道：「你不是已经领了送信的任务吗？还不快去做。",
                     '', 'songxin_xin')
    create_trigger_t('songxin_accept5',
                     "^>*\\s*褚万里\\D*说道：「你刚做完大理送信任务，还是去休息一会吧。",
                     '', 'songxin_rest')
    create_trigger_t('songxin_accept6',
                     "^>*\\s*褚万里交给你一封密函。", '',
                     'songxin_xin')
    create_trigger_t('songxin_accept7',
                     "^>*\\s*褚万里\\D*说道：「\\D*你并没有领了送信的任务，说什么放弃啊",
                     '', 'songxin_fangqi_heal')
    ...
    create_trigger_t('songxin_letter1', "^>*\\s*「(\\D*) (\\D*)」 亲启。",
                     '', 'songxin_consider')
    ...
    create_trigger_t('songxin_killer1',
                     "^>*\\s*你隐约感觉到有人围了过来。", '',
                     'songxin_killer')
    create_trigger_t('songxin_killer2',
                     "^>*\\s*(\\D*)说道：「小子，乖乖把密函交出来吧！！！」",
                     '', 'songxin_killer_1')
    create_trigger_t('songxin_killer3',
                     "^>*\\s*糟糕，又冲上来了个人！", '',
                     'songxin_killer_2')
    create_trigger_t('songxin_killer4',
                     "^>*\\s*(\\D*)说道：「师兄，点子硬得很，我来帮你！！！」",
                     '', 'songxin_killer_3')
    ...
    create_trigger_t('songxin2_check1',
                     "(> )*\\D*在你的耳边悄声说道：你赶紧把它送到「(\\D*)」的「(\\D*)」手上。",
                     '', 'songxin2_target')
    create_trigger_t('songxin2_check2',
                     "(> )*\\D*在你的耳边悄声说道：可能有个武功和你相比(\\D*)的家伙要来抢你",
                     '', 'songxin2_level')
    ...
    create_trigger_t('songxin2_killer1',
                     "^>*\\s*你定睛一看，原来是(\\D*)，而且此人\\D*，似乎用的是(\\D*)的(\\D*)！",
                     '', 'songxin2_check')
    create_trigger_t('songxin2_killer2',
                     "^(> )*卧室不能(吐纳|打坐)，会影响别人休息。",
                     '', 'songxin2_refuse')
    create_trigger_t('songxin2_killer3',
                     "^(> )*(你正要有所动作|你无法静下心来修炼|你还是专心拱猪吧)",
                     '', 'songxin2_refuse')
    create_trigger_t('songxin2_killer4',
                     "^(> )*这里不准战斗，也不准(吐纳|打坐)。",
                     '', 'songxin2_refuse')
    ...
    create_trigger_t('letteryichang1',
                     "^>*\\s*你紧张起来，不由一摸衣袋，不好！信被偷了!",
                     '', 'sx2over')
    create_trigger_t('letteryichang2',
                     "^>*\\s*你看了看信，决定还是不去了。", '',
                     'sx2overok')
...
function sxaskjob() return zhunbeineili(songxin_begin) end
...
function job_songxin1() exe('ask zhu wanli about job') end
...
function songxin_busy()
    ...
    if job.last == "wudang" then
        job.time.b = os.time()
        exe('cond')
        if condition.busy and condition.busy > 30 then
            return check_food()
        end
...
function songxin_rest()
    ...
    if job.zuhe["songmoya"] or job.zuhe["tdh"] then return huashan() end
    job.last = "songxin"
    return check_food()
end
...
function songxin_consider(n, l, w)
    ...
    if job.target == "边防官兵" and job.where == "嘉峪关东城门" then
        messageShow(
            '送信任务：任务目标【嘉峪关东城门为富不仁的可恨边防兵】是不会有人给他送信的。',
            'blue')
        return check_halt(songxin_fangqi)
    end
    if job.where == "武当山院门" then
        messageShow('送信任务：任务目标【' .. job.target ..
                        '任务地点【' .. job.where ..
                        '】容易出错，任务放弃。', 'blue')
        return check_halt(songxin_fangqi)
    ...
    if not job.room or not path_cal() or
        ...
        messageShow('送信任务：任务地点【' .. job.where ..
                        '】不可到达，任务放弃。', 'blue')
        return check_halt(songxin_fangqi)
    end
    -- end
...
function songxin_find_begin()
    ...
    create_trigger_t('songxin_begin1',
                     '^>*\\D*' .. job.target .. '\\((\\D*)\\)', '',
                     'sx1job_wait')
...
function songxin_find_npc()
    ...
    job.flag()
    exe('look')
    messageShow('送信任务：开始寻找【' .. job.target ..
                    '】的确切位置！')
    return find()
end
function songxin_find_go()
    ...
    job.flag()
    exe('look')
    messageShow('送信任务：开始寻找【' .. dest.area .. dest.room ..
                    '】的【' .. job.target .. '】，进行送信！')
    return find()
end
...
function songxin_finish()
    ...
    messageShowT('送信任务：任务完成！用时:【' .. job.time.over ..
                     '】秒')
...
function songxin_killer_1(n, l, w)
    ...
    DeleteTriggerGroup("songxin_fight")
    create_trigger_t('songxin_fight1',
                     '^>*\\s*\\D*' .. sxjob.killer1 .. '\\((\\D*)\\)', '',
                     'songxin_kill_1')
    create_trigger_t('songxin_fight2',
                     '^>*\\s*(\\D*)「啪」的一声倒在地上', '',
                     'songxin_kill_die')
    create_trigger_t('songxin_fight3',
                     '^>*\\s*(\\D*)神志迷糊，脚下一个不稳，倒在地上昏了过去。',
                     '', 'songxin_kill_faint')
    create_trigger_t('songxin_fight4', '^  (\\D*)封信件', '',
                     'songxin_letter')
    create_trigger_t('songxin_fight5', '^>*\\s*你想攻击谁？', '',
                     'songxin_kill_over')
    create_trigger_t('songxin_fight6',
                     '^>*\\s*你把 "action" 设定为 "check_letter" 成功完成。',
                     '', 'songxin_kill_check')
    create_trigger_t('songxin_fight7',
                     '^(> )*(\\D*)道：你躲得过初一躲不过十五，我们还会来的！',
                     '', 'songxin_kill_fail')
...
function songxin_killer_3(n, l, w)
    ...
    create_trigger_t('songxin_fight8',
                     '^>*\\s*\\D*' .. sxjob.killer2 .. '\\((\\D*)\\)', '',
                     'songxin_kill_2')
...
function songxin_kill_faint(n, l, w)
    if w[1] == sxjob.killer1 then exe('kill ' .. sxjob.id1) end
    if w[1] == sxjob.killer2 then exe('kill ' .. sxjob.id2) end
end
...
function songxin_kill_over()
    if sxjob.killer2 == '偷信杀手2' then sxkiller2 = 1 end
    if sxkiller1 == 1 and sxkiller2 == 1 then
        fight.time.e = os.time()
        fight.time.over = fight.time.e - fight.time.b
        messageShowT('送信任务：战斗用时:【' .. fight.time.over ..
                         '】秒。')
        EnableTriggerGroup("songxin_killer", false)
        kezhiwugongclose()
        job.killer = {}
...
function songxin_kill_get()
    for i = 1, 6 do exe('get letter from corpse ' .. i) end
    geta()
...
function songxin2_kill_check()
    ...
    if score.party and score.party == '普通百姓' and
        (dest.area == '燕子坞' or dest.area == '曼佗罗山庄' or dest.area ==
            '姑苏慕容') then
        messageShow('送信任务2：任务目标是【' .. job.where ..
                        '】的【' .. job.target .. '】，杀手等级为【' ..
                        job.level .. '】，开始去送信！')
        return songxin2_sendto()
    end
    if score.party and score.party == '姑苏慕容' and
        (dest.area == '燕子坞' or dest.area == '曼佗罗山庄' or dest.area ==
            '姑苏慕容') then
        messageShow('送信任务2：任务目标是【' .. job.where ..
                        '】的【' .. job.target .. '】，杀手等级为【' ..
    ...
    if AddrIgnores[dest.area] then
        messageShow('送信任务2：任务地点【' .. job.where ..
                        '】路太远了，任务拒绝。', 'blue')
        return go(songxin_fangqi, '大理城', '驿站')
    end
...
function songxin2_level(n, l, w) job.level = tostring(w[2]) end
function songxin2_consider()
    ...
    messageShow(
        '送信任务2：任务目标是【' .. job.where .. '】的【' ..
            job.target .. '】，杀手等级为【' .. job.level ..
            '】，开始前往！')
...
function songxin2_find_begin()
    ...
    if string.find(sxjob.ide, job.target) then
        messageShow('送信任务2：任务目标【' .. job.target ..
                        '】容易死，直送。')
        return songxin2_sendto()
    else
        -- if string.find(job.where, '蝴蝶谷') then return hudiegu() end
...
function sxCondconsider()
    if condition.busy and condition.busy > 60 then return huashan() end
    -- return check_halt(check_food)
...
function Sx2toTdh()
    ...
    songxin_triggerDel()
    if mydummy == true then return dummyfind() end
    if needdolost == 1 or job.name == 'songxin2' then return sx2overok() end
...
function songxin2_wait()
    ...
    exe('set wimpy 100;yun qi;yun jing;yun jingli;tuna ' .. hp.jingxue / 2)
end
...
faintFunc["songxin"] = "songxinFindAgain"
faintFunc["songxin2"] = "songxinFindAgain"
...
function songxin2_look()
    DeleteTriggerGroup("songxin_look")
    create_trigger_t('songxin_look1',
                     '^>*\\s*\\D*' .. sxjob.killer1 .. '\\((\\D*)\\)', '',
                     'songxin_kill_21')
...
function songxin2_kill()
    ...
    create_trigger_t('songxin_fight8',
                     ...
                     '', 'songxin2_fangqi')
    create_trigger_t('songxin_fight9',
    ...
    -- exe('jifa all;unset wimpy;set wimpycmd pfmpfm\\hp')
    exe(
        'kezhiwugongpfm;set wimpy 100;set wimpycmd pfmpower\\pfmpfm\\hp;yield no')
    kezhiwugong()
    kezhiwugongAddTarget(sxjob.killer1, sxjob.id1)
end
...
function sx1job_wait()
    ...
    messageShow('找到任务目标【' .. job.target ..
                    '】，开始打坐，等待Killer！')
    DeleteTriggerGroup("sx1lian")
    -- ain dls nv id dazuo
    create_trigger_t('liangong1',
    ...
    create_trigger_t('liangong2',
                     ...
                     '', 'sx1dazuobegin_desc')
    create_trigger_t('liangong3',
                     "^(> )*卧室不能(吐纳|打坐)，会影响别人休息。",
                     '', 'sx1songxin')
    create_trigger_t('liangong4',
                     "^(> )*(你正要有所动作|你无法静下心来修炼|你还是专心拱猪吧)",
                     '', 'sx1songxin')
    create_trigger_t('liangong5',
                     "^(> )*这里不准战斗，也不准(吐纳|打坐)。",
                     '', 'sx1songxin')
    create_trigger_t('liangong6',
                     "^(> )*这里可不是让你提高(内力|精力)的地方。",
                     '', 'sx1nodazuo')
    create_trigger_t('liangong7',
                     "^(> )*你吐纳完毕，睁开双眼，站了起来。",
                     '', 'sx1tuna')
    create_trigger_t('liangong8',
                     "^(> )*你的精力修为已经达到了你内功所能控制的极限。",
                     '', 'sx1tunaover')
...
sx1dazuobegin_desc = function() sx1wait = sx1wait + 1 end
sx1songxin = function() return songxin_find_go() end
sx1nodazuo = function() exe('yun qi;yun jing;yun jingli;jiali max;halt') end
sx1tuna = function() exe('yun jing;tuna ' .. hp.jingxue / 2) end

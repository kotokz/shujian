...
function check_pot(p_cmd)
    ...
    if score.party == "普通百姓" then
        ...
        for p in pairs(skills) do
            ...
            if q and q["force"] and perform.force and p == perform.force and skills[p].lvl < 100 then
...
function check_food_over()
    if
        ((kuang_cur and kuang_cur > 100000) or (kuang_cur1 and kuang_cur1 > 100000)) and condition.busy and
            condition.busy > 30
     then
        return Ronglian()
    end
...
function checkNxw()
    tmp.cnt = 0
    if
        score.gold and score.gold > 100 and
            (nxw_cur < count.nxw_max or cbw_cur < count.cbw_max or hqd_cur < count.hqd_max)
     then
...
function checkNxwi()
    if
        (nxw_cur < count.nxw_max or cbw_cur < count.cbw_max or hqd_cur < count.hqd_max) and Bag["黄金"] and
            Bag["黄金"].cnt > 4
     then

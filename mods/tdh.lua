...
function tdh_trigger()
    ...
    create_trigger_t('tdh_ask1', "^(> )*你向李式开打听有关", '',
                     'tdh_ask')
    create_trigger_t('tdh_ask2', "^(> )*这里没有这个人。$", '',
                     'tdh_nobody')
    ...
    create_trigger_t('tdh_accept1',
                     "^(> )*李式开在你的耳边悄声说道：速去", '',
                     'tdh_fangqi')
    create_trigger_t('tdh_accept2',
                     "^(> )*李式开说道：「既然你做不了，也就算了。」",
                     '', 'tdh')
    create_trigger_t('tdh_accept3',
                     "^(> )*李式开说道：「你又没领过任务，放弃什么啊？」",
                     '', 'tdh_lianxi')
    create_trigger_t('tdh_accept4',
                     "^(> )*李式开说道：「暂时没有事情需要做，以后再说吧。」",
                     '', 'tdh_lianxi')
    create_trigger_t('tdh_accept5',
                     "^(> )*李式开在你的耳边悄声说道：他的名字叫(\\D*)，你路上小心。",
                     '', 'tdh_consider')
    create_trigger_t('tdh_accept6',
                     "^(> )*李式开在你的耳边悄声说道：你马上去(\\D*)联络一个会里的兄弟。",
                     '', 'tdh_where')
    create_trigger_t('tdh_accept7',
                     "^(> )*李式开说道：「你刚完成任务，还是去休息会吧。」",
                     '', 'tdh_busy')
    create_trigger_t('tdh_accept8',
                     "^(> )*李式开说道：「你又没领过任务，放弃什么啊？」",
                     '', 'tdh_finish')
    create_trigger_t('tdh_accept9',
                     "^(> )*李式开白了你一眼，说：你无聊不无聊啊？",
                     '', 'tdh_join')
    create_trigger_t('tdh_accept10',
                     "^(> )*李式开说道：「你还在做任务呢！」",
                     '', 'songxin')
    create_trigger_t('tdh_accept11',
                     "^(> )*但是很显然的，李式开现在的状况没有办法给你任何答覆。",
                     '', 'tdh_nobody')
    create_trigger_t('tdh_accept12',
                     "^(> )*李式开说道：「你已经与(\\D*)联系上了，还是快点去和(\\D*)会合吧！」",
                     '', 'tdhFangqi')
    create_trigger_t('tdh_accept13',
                     "^(> )*李式开说道：「听说陈总舵主有事找你，你还是先去找总舵主询问吧。」",
                     '', 'tdhBug')
...
function tdhBug()
    ...
    dest.room = job.room
    dest.area = job.area
    create_trigger_t('tdh_find1', '^>*\\s*\\D*' .. job.target .. '\\((\\D*)\\)',
                     '', 'tdh_bug_ask')
...
function tdhFindAgain()
    ...
    if tmp.tdh then
        ...
        if tmp.tdh == 3 then EnableTrigger("tdh_find3", true) end
    end
...
function tdh_fangqi_go() return go(tdh_fangqi, '扬州城', '小金山') end
function tdh_fangqi()
    EnableTrigger("tdh_accept13", false)
    nobusy = 0
    EnableTriggerGroup("tdh_accept", false)
...
function tdh_consider(n, l, w)
    ...
    if job.where3 ~= nil then
        if getAddr(job.where3) ~= false then
            ...
            messageShow('天地会任务：任务地点【' .. job.where3 ..
                            '】不可到达，任务放弃。')
            return check_bei(tdh_fangqi_go)
        end
    else
        messageShow(
            '天地会任务：任务地点出错，【' .. job.where3 ..
                '】不存在，任务放弃。')
        return check_bei(tdh_fangqi_go)
    end
    -- job.room,job.area=getAddr(job.where3)
...
function tdh_find()
    DeleteTriggerGroup("tdh_find")
    create_trigger_t('tdh_find1',
                     '^>*\\s*\\D*' .. job.target3 .. '\\((\\D*)\\)', '',
                     'tdh_comrade')
    create_trigger_t('tdh_find2',
                     '^>*\\s*\\D*' .. job.target2 .. '\\((\\D*)\\)', '',
                     'tdhProtect')
    create_trigger_t('tdh_find3',
                     '^>*\\s*\\D*' .. job.target3 .. '\\((\\D*)\\)', '',
                     'tdhStop')
    create_trigger_t('tdh_find4',
                     "^(> )*你老远看见一人，样子恍惚和'..job.target3..'说的那人很像。$",
                     '', 'tdh_findok')
    create_trigger_t('tdh_find5',
                     '^>*\\s*' .. job.target3 .. '懒懒地伸了伸腰。', '',
                     'tdh_debug_find')
        ...
        if tmp.tdh == 3 then
            ...
            if tdh_npc1_room_id ~= nil and tdh_npc1_room_id ~= '' then
                if map.rooms[tdh_npc1_room_id].no_fight ~= nil then
                    target_room_is_no_fight =
                        map.rooms[tdh_npc1_room_id].no_fight -- 目标房间属于非战斗房间，最后一波npc可能刷不出来导致发呆！
                end
...
function tdh_find_act()
    job.flag()
    exe('look')
    messageShow('天地会任务：开始寻找【' .. dest.area .. dest.room ..
                    '】的革命同志！')
    return find()
end
function tdh_comrade(n, l, w)
    ...
    exe('follow ' .. job.id)
    locate()
    if road.id ~= nil then tdh_npc1_room_id = road.id end
...
function tdh_comrade_ask()
    DeleteTriggerGroup("tdh_cask")
    create_trigger_t('tdh_cask1', '^( )*' .. job.target3 ..
                         '说道：「在(\\D*)有一位叫做(\\D*)的兄弟，他想加入我们天地会。」',
                     '', 'tdhConsider')
    create_trigger_t('tdh_cask2', '^( )*' .. job.target3 ..
                         '白了你一眼，说：你无聊不无聊啊？', '',
                     'tdh_qiekou')
    create_trigger_t('tdh_cask3', '^( )*' .. job.target3 ..
                         '说道：「切口？哈哈，有点意思，你先说说',
                     '', 'tdh_qiekou')
    create_trigger_t('tdh_cask4', '^(> )*您先歇口气再说话吧。', '',
                     'tdh_askbusy')
...
function tdhConsider(n, l, w)
    ...
    if TdhIgnores[job.where2] then
        messageShow('天地会任务：任务地点【' .. job.where2 ..
                        '】不可到达，任务放弃。')
        return check_bei(tdh_fangqi_go)
    end
    return tdhcklol()
...
function tdhProtect(n, l, w)
    ...
    locate()
    if perform.force and perform.force == "dulong-dafa" then exe('yun wudi') end
    return check_bei(tdhProtectAsk, 1)
end
function tdhProtectAsk()
    if not locl.id[job.target2] then return check_bei(tdh_find) end
    DeleteTriggerGroup("tdh_fight")
    create_trigger_t('tdh_fight1',
                     '^(> )*(\\D*)(转眼间走的无影无踪。|「啪」的一声倒在地上，挣扎着抽动了几下就死了。)',
                     '', 'tdhDie')
    create_trigger_t('tdh_fight2',
                     '^(> )*(\\D*)怒道：“官府缉拿钦犯，闲杂人等闪开！”说罢便与' ..
                         job.target2 .. '战在了一起。', '', 'tdhProtectId')
    create_trigger_t('tdh_fight3',
                     '^(> )*(\\D*)叫道：“缉拿钦犯，闲杂人等闪开！”说罢便与你战在了一起。',
                     '', 'tdhProtectId')
    create_trigger_t('tdh_fight4', '^(> )*这里不准战斗。', '',
                     'tdhMoveOn')
...
function tdhProtectId(n, l, w)
    ...
    job.killer[w[2]] = true
    DeleteTriggerGroup("tdhId")
    create_trigger_t('tdhId1', '^( )*(\\D*)' .. w[2] .. '\\((\\D*)\\)', '',
                     'tdhId')
...
function tdhId(n, l, w)
    ...
    for p in pairs(job.killer) do job.killer[p] = string.lower(w[3]) end
...
function waithq()
    if tdhdz * 1 == 1 and tmp.kill <= 6 then
        wait.make(function()
            wait.time(1)
            exe('unset 积蓄;yun jing;yun qi;yun jingli;sxlian;dazuo ' ..
                    hp.dazuo)
...
function tdhPrepareneili()
    nobusy = 1
    if hp.neili < hp.neili_max * 1.2 then exe('eat ' .. drug.neili) end
    return prepare_neili(tdhPrepare)
end
function tdhPrepare()
    if dest.area == '燕子坞' or dest.area == '曼佗罗山庄' or dest.area ==
        '姑苏慕容' then
        if tmp.kill and tmp.kill > 6 then
            return tdhBack()
        elseif roundtdh > 2 then
            messageShow('天地会任务2：由于任务地点在【' ..
                            job.where2 .. '】，跳转地点【' .. roundtdh ..
                            '】次完毕，开始返回！')
            return tdhBack()
        elseif tdhround > 12 then
            tdhround = 1
            messageShow('天地会任务2：由于任务地点在【' ..
                            job.where2 .. '】，跳转地点第【' .. roundtdh ..
                            '】次！')
            roundtdh = roundtdh + 1
            return go(road.act)
        else
    ...
    if type(tdh_npc1_to_npc2_all_path) == 'string' then
        if string.find(tdh_npc1_to_npc2_all_path, 'jqgin') or
            string.find(tdh_npc1_to_npc2_all_path, 'jqgout') or
            string.find(tdh_npc1_to_npc2_all_path, 'yuRen') or
            string.find(tdh_npc1_to_npc2_all_path, 'Sld') or
            string.find(tdh_npc1_to_npc2_all_path, 'qu') then
            if tmp.kill and tmp.kill > 6 then
                return tdhBack()
            else
                return check_busy(tdhMove)
            end
        elseif string.find(tdh_npc1_to_npc2_all_path, 'duCjiang') or
            string.find(tdh_npc1_to_npc2_all_path, 'duHhe') then
        ...
        if not map.rooms[l_room].no_fight then
            for p in pairs(map.rooms[l_room].ways) do
                if dirReverse[p] then
                    ...
                    if map.rooms[r_room].no_fight then break end
                    if (not l_lengths or not l_lengths[p]) and
                        (not r_lengths or not r_lengths[q]) then
...
function tdhStop()
    ...
    EnableTrigger("hpheqi1", true)
    DeleteTriggerGroup("tdh_wait_npc2")
    create_trigger_t('tdh_wait_npc2_1', '^(> )*(\\D*)' .. job.target3 ..
                         '声嘶力竭地叫道，“大伙并肩子上啊！”。',
                     '', 'tdh_arrive_many')
    create_trigger_t('tdh_wait_npc2_2',
                     '^(> )*(\\D*)怒道：“官府缉拿钦犯，闲杂人等闪开！”说罢便与' ..
                         job.target2 .. '战在了一起。', '',
                     'tdh_last_Protect')
    create_trigger_t('tdh_wait_npc2_3',
                     '^(> )*(\\D*)叫道：“缉拿钦犯，闲杂人等闪开！”说罢便与你战在了一起。',
                     '', 'tdh_last_Protect')
    create_trigger_t('tdh_wait_npc2_4',
                     '^(> )*(\\D*)(转眼间走的无影无踪。|「啪」的一声倒在地上，挣扎着抽动了几下就死了。)',
                     '', 'tdhDie')
...
function tdh_arrive_many()
    ...
    if wait_times >= 1 then wait_times = wait_times - 1 end
    DeleteTriggerGroup("tdh_last_fight")
    create_trigger_t('tdh_last_fight1',
                     '^(> )*(\\D*)叫道：“缉拿钦犯，闲杂人等闪开！”说罢便与' ..
                         score.name .. '等战在了一起。', '',
                     'tdh_last_Protect')
...
function tdh_last_Protect(n, l, w)
    ...
    DeleteTriggerGroup("tdh_last_fight")
    DeleteTriggerGroup("tdh_last_Id")
    create_trigger_t('tdh_last_Id1', '^( )*(\\D*)' .. w[2] .. '\\((\\D*)\\)',
                     '', 'tdh_last_Id')
...
function tdhStopCheck()
    ...
    if not locl.id[job.target3] then return check_bei(tdh_find) end
    if wait_times > 6 and not locl.id[job.target2] then
        messageShow("天地会任务：判断【" .. job.target2 ..
                        "】没跟上！已经等待了6次，该回去【" ..
                        job.target2 .. "】寻找了！", 'red')
        wait_times = 0
        DeleteTimer("tdh_wait")
        DeleteTriggerGroup("tdh_goback_find_npc2")
        create_trigger_t('tdh_goback_find_npc2_1', '^(> )*' .. job.target2 ..
                             '(从\\D*走了过来。|紧跟着你快步走了过来。)',
                         '', 'tdhBack')
        SetTriggerOption("tdh_goback_find_npc2_1", "group",
                         "tdh_goback_find_npc2")
...
function tdh_finish()
    ...
    if score.party == "姑苏慕容" and need_dzxy == "yes" and
        string.find(jiangnan_area, locl.area) and hp.pot > 20 and hp.food > 50 and
        hp.water > 50 then
        messageShow("任务监控：天地会任务完成，当前区域【" ..
                        locl.area ..
                        "】，顺路检查是否去慕容领悟斗转星移！")
        return check_halt(checkdzxy)
    end
    if job.time.over <= 100 then
        return check_halt(wudang)
    else
        return check_halt(check_food)
    end
end

...
function wudangFail()
    ...
    if job.last == "songxin" or job.last == "songmoya" then
        ...
        if wudang_checkfood == 1 then
            ...
            if locl.weekday == "四" and (locl.hour == 6 or locl.hour == 7) and locl.min <= 55 then
                return songxin()
            end
            if
                mytime <= os.time() and SMYID[score.id] and score.xingyun and score.xiangyun ~= "衰" and
                    score.xiangyun ~= "死"
             then
                return songmoya()
            else
...
function wudangConsider(n, l, w)
    ...
    messageShow(
        "武当地点统计：神龙岛【" .. wdtosld .. "】次 ！慕容【" .. wdtomr .. "】次 ！燕子坞【" .. wdtoyzw .. "】次 ！曼佗罗山庄【" .. wdtomtl .. "】次 ！",
        "cyan"
    )
...
function wudangFind()
    ...
    create_trigger_t(
        "wudangFind3",
        "^(> )*\\D*" .. jobtarget2 .. "对着你发出一阵阴笑，说道：\\D*，这里地方太小，有种跟\\D*到外面比划比划！",
        "",
        "wudangLost"
    )
...
function wudangFindAct()
    ...
    messageShow(
        "武当任务：已到达任务地点【" ..
            job.where ..
                "】，开始寻找【" .. dest.area .. dest.room .. "】的" .. "【" .. jobtarget1 .. "】" .. "【" .. job.target .. "】！"
    )
end

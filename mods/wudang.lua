...
function wudangTrigger()
	...
	create_trigger_t('wudangAsk3',"^你觉得有点什么不对劲, 可是你却说不上来",'','wudangStart')
    ...
    create_trigger_t('wudangAccept1',"^(> )*宋远桥在你的耳边悄声说道：据说(\\D*)正在(\\D*)捣乱",'','wudangConsider')
    ...
    create_trigger_t('wudangAccept5',"^(> )*宋远桥说道：「你刚做完(武当锄奸|惩恶扬善|大理送信|抗敌颂摩崖|官府捕快|福州护镖)任务",'','wudangFail')
   ...
   setLocateRoomID='wudang/sanqing'
	...
	if job.last=="songxin" or job.last=="songmoya" then
		...
		if wudang_checkfood==1 then
			...
			if locl.weekday == '四' and (locl.hour==6 or locl.hour ==7 ) and locl.min<=55 then return songxin() end
			if mytime<=os.time() and SMYID[score.id] and score.xingyun and score.xiangyun~='衰' and score.xiangyun~='死' then
			   return songmoya()
			else
			   return songxin()
			end
...
function wudangConsider(n,l,w)
	...
	if string.find(job.where,"嵩山少林菜地") then 
		job.where = '嵩山少林寺前广场'
		messageShow('地点由嵩山少林菜地改为嵩山少林寺前广场','violet')
	end
...
function wudangFindGo()
	...
	pfmjineng()
	setLocateRoomID='wudang/sanqing'
    if not job.room or not path_cal() or string.find(job.where,'绝情谷石窟') then
...
function wudangFind()
    ...
    create_trigger_t('wudangFind3',"^(> )*\\D*"..jobtarget2.."对着你发出一阵阴笑，说道：\\D*，这里地方太小，有种跟\\D*到外面比划比划！",'','wudangLost')
...
function wudangFindAct()
    ...
    exe('look')
    find()
    messageShow('武当任务：已到达任务地点【'..job.where..'】，开始寻找【'..dest.area .. dest.room ..'】的'..'【'.. jobtarget1 ..'】'..'【'.. job.target ..'】！')
end
...
function wudangLost(n,l,w)
    ...
    if job.lost>3 then
       job.lost=0
	jobfailLog()
    messageShow('武当任务：搜索丢失【'.. job.target ..'】三次！回去放弃！',"Plum")
       return check_halt(wudangFindFail)
    end
    --if job.id==Trim(w[2]) then
       dis_all()
    EnableTriggerGroup("wudangdebug",true)
	messageShow('武当任务：搜索丢失【'.. job.target ..'】！回头再找！',"white")
...
fight.time.b=os.time()
    ...
    create_trigger_t('wudangFight5',"^(> )*加油！加油！",'','wdkill_timer_stop')	
end

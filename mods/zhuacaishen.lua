...
最后，祝人人发大财！！！黄金爆满仓！！！]] --------by 桃花岛无法风（2019.1.1）---------
...
function caishen_location(n,l,w)
...
end]] function zhuacaishen_find()
    job.name = 'zhuacaishen'
    flag.find = 0
    DeleteTriggerGroup("zhuacaishen_find")
    DeleteTrigger("zhuacaishen_find_success")
    create_trigger_t('zhuacaishen_find1',
                     '^(> )*\\s*财神 邯郸爷 赵公明\\(Zhao gongming\\)',
                     '', 'caishenTarget')
    create_trigger_t('zhuacaishen_find_success',
                     "^(> )*财神爷为你增加了(\\D*)錠黄金存款", '',
                     'zhuacaishen_finish')
    create_trigger_t('zhuacaishen_find2',
                     '^(> )*(这里没有 zhao gongming|这里没有 zhao)', '',
                     'zhuacaishen_goon')
    ...
    l_result = utils.inputbox("你要抓的财神位于？", "财神位置",
                              GetVariable("caishen_location"), "宋体", "12")
    if not isNil(l_result) then SetVariable("caishen_location", l_result) end
...
function caishenFindAgain() return go(zhuacaishen_FindAct, dest.area, dest.room) end
function caishenFindFail()
    wdgostart = 0
    messageShow(
        '抓财神任务：搜索丢失【财神同志】三次！请等待rumor提示，再来抓，祝你好运！',
        'white', 'black')
...
function zhuacaishen_FindAct()
    ...
    exe('look')
    find()
    messageShow('抓财神：已到达任务地点【' ..
                    GetVariable("caishen_location") ..
                    '】，开始寻找【财神】', 'lime', 'black')
end

...
function checkPrepareOver()
  if lostletter==1 and needdolost==1 then
    ...
    vippoison=0
     exe('score;cond')
  if wudang_checkfood==1 or (condition.busy and condition.busy > 10) or needxuexi==1 then
     return check_xuexi()
  else
     return check_job()
  end
...
function check_food_over()
   if (kuang_cur and kuang_cur>2000) or (kuang_cur1 and kuang_cur1>2000) then return Ronglian() end
      return check_heal()
end
...
function xuezhuTrigger()
    ...
    create_trigger_t('xuezhuFight3',"^(> )*雪蛛神志迷糊，脚下一个不稳，倒在地上昏了过去。",'','getxuezhu')
...
function getxuezhu0()
	xuezhuTrigger()
	if inwdj==0 then
		messageShow('抓雪蛛：苗疆地图不可到达，任务放弃。',"Plum")
...
function xuezhu_go()
	...
	if inwdj==0 then
		messageShow('抓雪蛛：苗疆山洞不可到达，任务放弃。',"Plum")
		SetVariable("xuezhu_status","2")
		return check_halt(checkPrepare)
	end
	EnableTriggerGroup("xuezhuAccept",false)
...
function turnShen(x)
    if x and x=='+' then
	   if hp.shen<=-10000 then
	      if job.zuhe["wudang"] then job.zuhe["wudang"]=job.zuhe["xueshan"] end
	      return xueshan()
	   end
	...
	if x and x=='-' then
	   if hp.shen>=10000 then
	      if job.zuhe["xueshan"] then job.zuhe["xueshan"]=job.zuhe["wudang"] end
	         return wudang()
	   end
